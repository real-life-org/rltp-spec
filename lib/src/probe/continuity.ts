...
async function handleProbe (p: Person, { key, t }: any, doc: any, when: number, ent: any) {
  ...
  if (t.probeAsm?.probe !== doc.probe) t.probeAsm = { probe: doc.probe, blinded: [], next: 1 }
  if (Number(doc.seq) !== t.probeAsm.next) return { handled: true, error: 'malformed probe (chunk order)' }
  t.probeAsm.next += 1
