 ...
 */
export async function verifyAck (p: Person, contact: any, doc: any, cls: 'signature' | 'dv' = 'dv'): Promise<{ ref: string, threadId: string } | null> {
  try {
    ...
    if (cls === 'signature') {
      if (doc.proof.mac !== undefined || doc.proof.type !== 'DataIntegrityProof' || doc.proof.cryptosuite !== 'eddsa-jcs-2022') return null
      if (!C.calOK(doc.proof.created)) return null
    } else {
    ...
    if (cls === 'signature') {
      ...
      if (!(await C.diVerify(doc, doc.issuer))) return null
