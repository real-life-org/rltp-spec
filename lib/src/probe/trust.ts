...
export async function receiveTrustDoc (p: Person, env: any, when: number, ent: any = {}) {
  ...
  if (doc?.type === ACK_TYPE) {
    ...
    if (doc.recipient !== from.channel.own.anchor) return { handled: true, error: 'outer binding (recipient)' }
    return withContactLock(from, async () => {
      if (from.deactivated) return { handled: true, error: 'tuple deactivated' }   // Re-Check nach dem Lock-Erwerb
...
async function receiveAckInner (p: Person, fromKey: string, from: any, opened: any) {
  ...
  if (from.deactivated) return { handled: true, error: 'tuple deactivated' }   // Re-Check nach den Awaits (Review 37, B-3), danach kein await
  const o = outboxOf(from).get(v.ref)
  if (!o || o.threadId !== v.threadId) return { handled: true, error: 'ack unknown ref' }
  ...
  else outboxOf(from).delete(v.ref)
  C.effectDone(p, opened.digest)
  if (o.kind === 'star') {
    const s = subOf(from)
