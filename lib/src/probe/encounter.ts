...
const sec = (ms: number) => Math.floor(ms / 1000)                 // 2.3: whole seconds
...
const uuidOk = (v: unknown) => typeof v === 'string' && UUID_V4.test(v)
const displayName = (p: Person) => String(p.name ?? '').slice(0, 200)
const secIso = (iso: string) => Math.floor(Date.parse(iso) / 1000)
...
const chal = (p: Person): Map<string, any> => ((p as any).challenges ??= new Map())
const recs = (p: Person): Map<string, any> => ((p as any).records ??= new Map())
...
export function resolve (p: Person, value: string, now: number): 'recorded' | 'open' | 'unknown' {
  if (recs(p).has(value)) return 'recorded'
  const c = chal(p).get(value)
  if (!c || c.aged) return 'unknown'
  if (sec(now) > sec(c.tCh) + (CHALLENGE_MAX_AGE + SKEW_TOLERANCE) / 1000) { c.aged = true; return 'unknown' }  // latch
  return 'open'
}
const futureOk = (tCh: number, now: number) => sec(tCh) <= sec(now) + SKEW_TOLERANCE / 1000   // 5.5 explicit future check
// the producer's mirror of 5.6 step 6: may a credential binding a challenge issued at tChMs still be issued now?
const issuanceOpen = (tChMs: number, now: number) => { const skew = SKEW_TOLERANCE / 1000; return sec(now) >= sec(tChMs) - skew && sec(now) <= sec(tChMs) + (CHALLENGE_MAX_AGE + ISSUANCE_WINDOW) / 1000 + skew }

// ── the LOCK SET of Delivery 6.2 stage 9 ────────────────────────────────
// Atomic acquisition of every key (single-threaded: no await between the
// check and the marks); a waiter holds nothing; when the way is free the
// caller's body runs as the re-entry at stage 4.
type LockState = { held: Set<string>, waiters: Array<() => void> }
const lockState = (p: Person): LockState => ((p as any).lockSet ??= { held: new Set(), waiters: [] })
const release = (L: LockState, keys: string[]) => { for (const k of keys) L.held.delete(k); const w = L.waiters; L.waiters = []; for (const r of w) r() }
/** acquire the whole set at once if free (synchronous); else null */
const tryAcquire = (p: Person, keys: string[]): LockState | null => {
  const L = lockState(p)
  if (keys.some((k) => L.held.has(k))) return null
  for (const k of keys) L.held.add(k)
  return L
}
const awaitRelease = (p: Person) => new Promise<void>((res) => lockState(p).waiters.push(res))
/** producer-side critical section (own actions, no stage evaluation): wait, then run */
async function withLockSet<T> (p: Person, keys: string[], fn: () => Promise<T>): Promise<T> {
  let L = tryAcquire(p, keys)
  while (!L) { await awaitRelease(p); L = tryAcquire(p, keys) }
  try { return await fn() } finally { release(L, keys) }
}

// ── the ONE commit discipline ───────────────────────────────────────────
// prepare (async, no mutation, UNLOCKED — stages 4–8) → lock set acquired
// (stage 9) → [test hook] → verify (sync) → commit (sync) → release.
// verify returns null (go), 'retry' (the state moved: release and RE-ENTER
// AT STAGE 4 — prepare runs again under the new state, no lock held across
// the re-entry, unbounded as 6.2 demands), or a disposition string.
// prepare may short-circuit with { done } (a result needing no commit) or
// { error }.
type Prep<P> = { error?: string, done?: any, prep?: P }
type Effect<P, R> = {
  prepare: () => Promise<Prep<P>>,
  verify: (prep: P) => null | 'retry' | string,
  commit: (prep: P) => R,
}
async function effect<P, R> (p: Person, keys: string[], e: Effect<P, R>, fail: (err: string) => R): Promise<R> {
  for (;;) {
    const pr = await e.prepare()                       // stages 4–8, holding nothing
    if (pr.error !== undefined) return fail(pr.error)
    if (pr.done !== undefined) return pr.done
    ...
    if (!L) { await awaitRelease(p); continue }
    try {
      const hook = (p as any).__beforeVerify
      if (typeof hook === 'function') hook()
      // ── from here to the end of commit: NO await ──
      const v = e.verify(pr.prep as P)
      if (v === 'retry') continue                        // the state moved: re-enter at stage 4
      if (v !== null) return fail(v)
      return e.commit(pr.prep as P)
    } finally { release(L, keys) }
  }
}
const failed = (err: string) => ({ handled: true, error: err }) as any

// ── cards (Encounter 6): schema-valid, decoded keys, proof under the anchor
const CARD_SCHEMA = 'contact-card-0.25.schema.json'
async function cardOk (card: any): Promise<string | null> {
  if (!schemaOk(CARD_SCHEMA, card)) return 'card fails contact-card-0.25 schema'
  if (card.version !== C.CARD_VERSION) return 'card version unknown'
  if (!card.challenge) return 'card without challenge (enactment use requires one)'
  if (!C.calOK(card.challenge.issuedAt) || !C.calOK(card.proof?.created)) return 'card: calendar-invalid timestamp'
  if (!C.xRawOfMk(card.keyAgreement) || !C.edRawOfAnchor(card.anchor)) return 'card keyAgreement does not decode (x25519-pub + 32 bytes, 2.3)'
  if (!(await C.diVerify(card, card.anchor))) return 'card proof'
  return null
}

// ── show: a displayed card (5.3 displayed challenge) ─────────────────────
export async function show (p: Person, when: number, ent: any = {}) {
  const ctx = await C.pairContext(p.rootIkm, ent.nonce ?? C.rand(32))
  if (p.contexts.has(ctx.anchor)) return { error: 'validation-failed (pair anchor already used — fresh-always, 4.4)' } as any
  ...
  if (recs(p).has(ch) || chal(p).has(ch)) return { error: 'validation-failed (challenge value already issued)' } as any
...
const edgeState = (t: any) => t.credential && t.credentialIn ? '✓' : t.credential ? '→' : t.credentialIn ? '←' : '◇'
function buildTuple (p: Person, ownCtx: any, peerCard: any, ownValue: string, peerValue: string, bind: string, when: number, extra: any = {}) {
  const tuple: any = {
...
function commitRecord (p: Person, tuple: any, when: number) {
...
// ── outbox / threads / acks ─────────────────────────────────────────────
const outboxOf = (t: any) => (t.outbox ??= new Map<string, any>())
...
const threadInUse = (p: Person, id: string) => {
  for (const t of p.contacts.values()) {
    if (t.bundleThread === id || t.inThreads?.has(id)) return true
    for (const o of (t.outbox ?? new Map()).values()) if (o.threadId === id) return true
  }
  return false
}
async function issueDoc (p: Person, tuple: any, type: string, payload: any, when: number, threadId: string, kind: string, ent: any = {}, ceremony?: any, onCommit?: () => void) {
  const doc: any = { id: ent.id ?? globalThis.crypto.randomUUID(), type, issuer: tuple.channel.own.anchor, recipient: tuple.card.anchor, threadId, issuedAt: C.iso(when), payload }
  if (ceremony) doc.ceremony = ceremony
  ...
  if (!uuidOk(doc.id) || !uuidOk(threadId) || !schemaOk('rltp-delivery-document.schema.json', doc)) throw new Error('document profile violated (id/threadId UUID v4, schema)')
...
const mkAck = (p: Person, tuple: any, digest: string, threadId: string, when: number, recipient?: string) => buildAck(p, tuple, digest, threadId, when, { cls: 'signature', ...(recipient ? { recipient } : {}) }).then((a: any) => a.env)
...
const retained = (p: Person): Map<string, { env: any, to: any }> => ((p as any).ackRetained ??= new Map())
const commitAck = (p: Person, tuple: any, digest: string, ackEnv: any) => {
...
const dupResult = (p: Person, digest: string, anchor: string) => {
...
const cached = (p: Person, digest: string) => (p.deliveryCache ?? new Set()).has(digest)
...
export async function scan (p: Person, peerCardIn: any, when: number, ent: any = {}) {
  ...
  if (ce) return { error: `displayed card invalid (${ce})` }
  if (peerCard.sentTo !== undefined || peerCard.boundTo !== undefined) return { error: 'a displayed card carries neither sentTo nor boundTo' }
  const ctx = await C.pairContext(p.rootIkm, ent.nonce ?? C.rand(32))
  if (p.contexts.has(ctx.anchor)) return { error: 'validation-failed (pair anchor already used — fresh-always, 4.4)' }
  let ownValue = C.challengeOf(ent.ch ?? C.rand(17))
  while (!ent.ch && ownValue === peerCard.challenge.value) ownValue = C.challengeOf(C.rand(17))
  if (ownValue === peerCard.challenge.value) return { error: 'validation-failed (c_A equals the displayed challenge — never reused in a sent card, 5.3)' }
  ...
  const alreadyScanned = () => { for (const r of recs(p).values()) if (r.peerValue === peerValue && r.counterpart === peerCard.anchor) return true; return false }
  if (alreadyScanned()) return { error: 'validation-failed (this code was already scanned — its challenge is in a record, 5.3)' }
  const displayKey = 'display:' + peerCard.anchor + ':' + peerValue
  const threadId = ent.threadId ?? globalThis.crypto.randomUUID()
  if (!uuidOk(threadId) || (ent.id !== undefined && !uuidOk(ent.id))) return { error: 'validation-failed (id/threadId must be UUID v4 — Delivery 3)' }   // before the record gate
  const iso = C.iso(when)
  const recorded = await effect<any, any>(p, [ownValue, displayKey], {
    prepare: async () => ({ prep: { bind: await C.binding(C.CEREMONY, ownValue, peerValue) } }),
    verify: () => {
      if (alreadyScanned()) return 'validation-failed (this code was already scanned — its challenge is in a record, 5.3)'
      if (recs(p).has(ownValue) || chal(p).has(ownValue)) return 'validation-failed (own challenge value already in use)'   // single use (5.3/5.5)
      if (p.contexts.has(ctx.anchor)) return 'validation-failed (pair anchor already used — fresh-always, 4.4)'
      if (threadInUse(p, threadId)) return 'validation-failed (threadId not fresh — a bundle opens its exchange, 4.1)'
      if (!futureOk(when, when)) return 'gate-future'
      return null
    },
    commit: ({ bind }) => {
      p.contexts.set(ctx.anchor, ctx); p.contexts.set(ctx.keyAgreement, ctx)
      chal(p).set(ownValue, { tCh: when, ctxAnchor: ctx.anchor })        // enters as OPEN, t_ch = now …
      if (resolve(p, ownValue, when) !== 'open') { chal(p).delete(ownValue); return { error: 'validation-failed (own challenge does not resolve open)' } }   // … resolved AUTHORITATIVELY in the same span
  ...
  if (!out) return { error: 'stale-issuance (issuance window closed before the credential could be issued)' }
  return { ...out, ctx }
}
...
async function completeScan (p: Person, tuple: any, when: number, ent: any = {}) {
  ...
  if (!tuple.credential) {
    if (!issuanceOpen(Date.parse(tuple.card.challenge.issuedAt), when)) { delete tuple.bundlePending; tuple.bundleLapsed = 'stale-issuance'; say(p, `${tuple.name}: Ausstellung nach dem Issuance-Window — kein Credential mehr (Record bleibt)`); return null }
    ...
    if (!tuple.credential) { tuple.credential = cred; rec.accepted.out = d; tuple.state = edgeState(tuple) }
  }
  if (!tuple.sentCard) {
  ...
  if (existing) { delete tuple.bundlePending; return { to: tuple, kind: 'encounter-bundle/0.1', env: structuredClone(existing[1].env), threadId: existing[1].threadId, digest: existing[0], sentCard: tuple.sentCard, credential: tuple.credential } }
 ...
 */
export async function resumeEncounter (p: Person, when: number) {
  const outbound: any[] = []
  for (const [anchor, t] of [...p.contacts]) {
    if (!t.ownChallenge) continue
    // only STANDING duties survive deactivation: an owed bundle (record
    // stands, 5.8 step 3) and an owed counter document (5.8 step 4)
    if (t.deactivated && !(t.credential && t.counterDocPending) && !t.bundlePending) continue
    try {
      if (t.bundlePending) { const o: any = await withLockSet(p, [t.ownChallenge], async () => t.bundlePending ? completeScan(p, t, when) : null); if (o?.env) outbound.push({ to: t, kind: o.kind + ' (resumed)', env: o.env, sentCard: o.sentCard, threadId: o.threadId, peerAnchor: t.card.anchor }) }
      if (t.counterDocPending && t.bundleThread) { const o: any = await counterEffect(p, anchor, when, {}, [t.ownChallenge]); if (o?.env) outbound.push({ to: t, kind: o.kind + ' (resumed)', env: o.env }) }
    } catch (err) { say(p, `resume: ${t.name}: ${String((err as any)?.message ?? err)} — bleibt ausstehend`) }
  }
  return { outbound }
}
...
export async function counter (p: Person, peerAnchor: string, when: number, ent: any = {}) {
  if (ent.id !== undefined && !uuidOk(ent.id)) return { error: 'validation-failed (id must be UUID v4 — Delivery 3)' }
  const t0 = p.contacts.get(peerAnchor)
  if (!t0?.ownChallenge) return { error: 'no enactment record for this contact' }
  ...
  if (t0.deactivated && !(t0.credential && t0.counterDocPending)) return { error: 'no enactment record for this contact' }
...
async function counterEffect (p: Person, peerAnchor: string, when: number, ent: any, keys: string[]) {
...
async function counterBody (p: Person, peerAnchor: string, when: number, ent: any) {
  const issued = await effect<any, any>(p, [], {
    prepare: async () => {
      const t = p.contacts.get(peerAnchor)
      if (!t?.ownChallenge || !recs(p).has(t.ownChallenge)) return { error: 'no enactment record for this contact' }
      if (t.credential && !t.counterDocPending) return { error: 'counter-step already issued' }
      if (t.credential) return { prep: { t, cred: t.credential, fresh: false } }   // materialization — allowed even when deactivated
      if (t.deactivated) return { error: 'no enactment record for this contact' }   // no NEW issuance on a deactivated tuple
      ...
      if (!issuanceOpen(Date.parse(t.card.challenge.issuedAt), when)) return { error: 'stale-issuance (issuance window closed — no counter-step any more)' }
    ...
    verify: ({ t, fresh }) => {
      if (!recs(p).has(t.ownChallenge)) return 'no enactment record for this contact'
      if (fresh && t.deactivated) return 'no enactment record for this contact'
      if (fresh && t.credential) return 'counter-step already issued'   // a concurrent issue won
      return null
    },
    commit: ({ t, cred, d, fresh }) => {
      if (fresh) { t.credential = cred; recs(p).get(t.ownChallenge).accepted.out = d; t.counterDocPending = true; t.state = edgeState(t) }
      return { t }
    },
  }, (err) => ({ error: err }))
  ...
  if (existing) { delete t.counterDocPending; return { to: t, kind: 'encounter-credential-delivery/0.1', env: structuredClone(existing[1].env), threadId: existing[1].threadId, digest: existing[0], credential: t.credential } }
...
export async function captureSentCard (p: Person, cardIn: any, when: number) {
  const card = structuredClone(cardIn)   // SNAPSHOT before the first await (TOCTOU): verified = recorded
  const ce = await cardOk(card)
  if (ce) return { error: `sent card invalid (${ce})` }
  if (typeof card.sentTo !== 'string' || typeof card.boundTo !== 'string') return { error: 'not a sent card' }
  if (card.challenge.value === card.boundTo) return { error: 'validation-failed (sent challenge equals the displayed challenge — 5.3)' }
  const ctx = p.contexts.get(card.sentTo)
  if (!ctx || ctx.anchor !== card.sentTo) return { error: 'sentTo ≠ own anchor' }
  // the 5.5 taxonomy for a bound challenge that resolves `recorded`
  const recordedTaxonomy = (): string | { handled: true, idempotent: true, to: string } => {
    const r = recs(p).get(card.boundTo)
    if (r.ctxAnchor !== ctx.anchor) return 'validation-failed (challenge bound to another anchor)'
    if (r.counterpart !== card.anchor) return 'consumed-challenge'
    if (jcs(r.card) !== jcs(card)) return 'validation-failed (card differs from record)'
    return { handled: true, idempotent: true, to: card.anchor }
  }
  return effect<any, any>(p, [card.boundTo], {
    ...
    verify: (q) => {
      ...
      if (st === 'unknown') return 'gate-expired'
      if (st === 'recorded') { const r = recordedTaxonomy(); if (typeof r === 'string') return r; q.idem = true; return null }
      if (chal(p).get(card.boundTo).ctxAnchor !== ctx.anchor) return 'validation-failed (challenge bound to another anchor)'   // 4.4: sentTo = the anchor that displayed c_B
      if (!futureOk(chal(p).get(card.boundTo).tCh, when)) return 'gate-future'
      return null
    },
    commit: ({ bind, idem }) => {
      if (idem) return { handled: true, idempotent: true, to: card.anchor }   // JCS-identical card on the existing record: no-op
...
export async function receiveEncounter (p: Person, env: any, when: number, ent: any = {}) {
  ...
  if (doc?.type !== BUNDLE_TYPE && doc?.type !== DELIVERY_TYPE) return { handled: false, doc }
  if (opened.duplicate) return reAck(p, opened.digest!)
  ...
  if (doc.recipient !== ctx.anchor) return { handled: true, error: 'wrong-recipient' }   // 6.2 stage 5 disposition
...
function reAck (p: Person, digest: string) {
  const r = retained(p).get(digest)
  if (r) return { handled: true, duplicate: true, ack: { to: r.to, kind: 'delivery-ack/0.1 (Re-Ack, byte-identisch)', env: structuredClone(r.env) } }
  for (const t of p.contacts.values()) { const stored = t.ackStore?.get(digest); if (stored) return { handled: true, duplicate: true, ack: { to: t, kind: 'delivery-ack/0.1 (Re-Ack, byte-identisch)', env: structuredClone(stored) } } }
...
function credFormat (cred: any, ceremonyHere = true): string | null {
  ...
  if (!C.calOK(cred.validFrom) || !C.calOK(cred.proof?.created)) return 'ERR_VERSION'
  if (!sameDigest(cred.credentialSubject.enactmentBinding, cred.credentialSubject.enactmentBinding)) return 'ERR_VERSION'
  if (!C.edRawOfAnchor(cred.issuer) || !C.edRawOfAnchor(cred.credentialSubject.id)) return 'ERR_VERSION'
  const vm = String(cred.proof?.verificationMethod ?? ''), hash = vm.indexOf('#')
  if (hash < 0 || !C.edRawOfAnchor(vm.slice(0, hash)) || !C.edRawOfAnchor('did:key:' + vm.slice(hash + 1))) return 'ERR_VERSION'
  return null
}
// steps 1–3: format · signature · addressee
async function credHead (cred: any, ctx: any, ceremonyHere = true): Promise<string | null> {
  const f = credFormat(cred, ceremonyHere)
  if (f) return f
  if (!(await C.diVerify(cred, cred.issuer))) return 'ERR_SIG'
  if (cred.credentialSubject.id !== ctx.anchor) return 'ERR_ADDRESSEE'
  return null
}
// step 6: the issuance window, whole seconds, inclusive, skew widens
function windowOk (cred: any, tChMs: number): boolean {
...
function acceptTail (cred: any, rec: any, order: 'credential' | 'bundle'): string | null {
  if (!rec || rec.counterpart !== cred.issuer) return 'ERR_NO_RECORD'
  if (cred.credentialSubject.ceremony !== rec.ceremony) return 'ERR_CEREMONY'
  const bindOk = sameDigest(cred.credentialSubject.enactmentBinding, rec.bind)
  if (order === 'bundle' && !bindOk) return 'ERR_BINDING'
  if (!windowOk(cred, rec.tCh)) return 'ERR_STALE_ISSUANCE'
  if (!bindOk) return 'ERR_BINDING'
  return null
}
const disposition = (e: string) => e === 'ERR_STALE_ISSUANCE' ? 'stale-issuance' : `validation-failed (${e})`
...
const sameOpeningBundle = (p: Person, anchor: string, threadId: string, digest: string) => {
  const t = p.contacts.get(anchor)
  if (!t) return false
  if (t.bundleThread === threadId) return sameDigest(t.bundleDigest, digest)
  const d = t.inThreadDigests?.get(threadId)
  return d !== undefined && sameDigest(d, digest)
}
async function bundleChecks (p: Person, ctx: any, doc: any, digest: string, when: number): Promise<any> {
  const pl = doc.payload
  if (!schemaOk('payload-encounter-bundle.schema.json', pl)) return { error: 'malformed bundle' }   // stage 7
  ...
  if (!C.calOK(doc.issuedAt) || !C.calOK(card.challenge?.issuedAt) || !C.calOK(card.proof?.created)) return { error: 'calendar-invalid issuedAt' }
  if (doc.issuer !== card.anchor || card.anchor !== credential.issuer) return { error: 'outer binding (issuer = card.anchor = credential.issuer)' }
  if (credential.credentialSubject?.id !== doc.recipient) return { error: 'outer binding (recipient = subject)' }
  if (card.sentTo !== doc.recipient || card.boundTo !== credential.credentialSubject.challenge) return { error: 'sent card binding (sentTo/boundTo)' }
  if (card.challenge.value === card.boundTo) return { error: 'validation-failed (sent challenge equals the displayed challenge — 5.3)' }
  if (doc.ceremony?.enactment !== undefined && !sameDigest(doc.ceremony.enactment, credential.credentialSubject.enactmentBinding)) return { error: 'ceremony.enactment recomputes not (validation-failed)' }
  if (card.version !== C.CARD_VERSION || !card.challenge) return { error: 'validation-failed (card version/challenge)' }
  if (!C.xRawOfMk(card.keyAgreement) || !C.edRawOfAnchor(card.anchor)) return { error: 'validation-failed (card keys do not decode)' }
  if (threadInUse(p, doc.threadId) && !sameOpeningBundle(p, card.anchor, doc.threadId, digest)) return { error: 'validation-failed (threadId not fresh — a bundle opens its exchange)' }
  const f = credFormat(credential, false)
  if (f) return { error: disposition(f) }
  // (2) card proof · (3) credential proof · (4) addressee
  if (!(await C.diVerify(card, card.anchor))) return { error: 'card proof' }
  const h = await credHead(credential, ctx, false)
  if (h) return { error: disposition(h) }
  ...
  if (st !== 'unknown') {
    if (credential.credentialSubject.ceremony !== C.CEREMONY) return { error: 'validation-failed (ceremony)' }
    ...
    if (ctxOfChallenge(p, own, st) !== ctx.anchor) return { error: 'validation-failed (challenge bound to another anchor)' }
    ...
    if (!sameDigest(credential.credentialSubject.enactmentBinding, bind)) return { error: 'validation-failed (ERR_BINDING)' }
    if (!windowOk(credential, tCh)) return { error: 'stale-issuance' }
  }
  return { own, bind, card, credential, st }
}
const ctxOfChallenge = (p: Person, value: string, st: 'open' | 'recorded') => st === 'recorded' ? recs(p).get(value)?.ctxAnchor : chal(p).get(value)?.ctxAnchor

async function receiveBundle (p: Person, ctx: any, doc: any, digest: string, when: number, ent: any) {
  const pre = await bundleChecks(p, ctx, doc, digest, when)
  if (pre.error) return failed(pre.error)
  const { own, card } = pre
  // stage 9 under the LOCK SET {digest, record key}
  const res: any = await effect<any, any>(p, [digest, own], {
    prepare: async () => {
      if (cached(p, digest)) return { done: dupResult(p, digest, card.anchor) }
      ...
      if (r.st === 'unknown') return { prep: { ...r, credDigest, mode: 'unknown' } }
      if (r.st === 'open') {
    ...
    verify: (q) => {
      if (cached(p, digest)) return 'retry'                       // a concurrent identical document committed → duplicate-known
      const st = resolve(p, own, when)                              // AUTHORITATIVE — same synchronous span as the commit
      if (st === 'unknown') return 'validation-failed (challenge unknown)'
      if (st !== q.mode) return 'retry'                             // the state moved: re-enter at stage 4
      if (ctxOfChallenge(p, own, st) !== ctx.anchor) return 'validation-failed (challenge bound to another anchor)'
      if (threadInUse(p, doc.threadId) && !sameOpeningBundle(p, card.anchor, doc.threadId, digest)) return 'validation-failed (threadId not fresh — a bundle opens its exchange)'
      if (st === 'open') {
        if (!futureOk(chal(p).get(own).tCh, when)) return 'gate-future'
      ...
      if (rec.counterpart !== card.anchor) return 'consumed-challenge'
      if (jcs(rec.card) !== jcs(card)) return 'validation-failed (card differs from record)'
      const e = acceptTail(q.credential, rec, 'bundle')
      if (e) return disposition(e)
      if (rec.accepted.in && !sameDigest(rec.accepted.in, q.credDigest)) return 'validation-failed (ERR_CONFLICT)'
      if (!q.tuple || !q.ackEnv || p.contacts.get(card.anchor) !== q.tuple) return 'retry'   // tuple moved since prepare: re-enter
      return null
    },
    commit: (q) => {
      if (q.mode === 'open') {
      ...
      tuple.credentialIn = q.credential; tuple.bundleThread ??= doc.threadId; tuple.bundleDigest ??= digest
      if (tuple.bundleThread !== doc.threadId) { (tuple.inThreads ??= new Set()).add(doc.threadId); (tuple.inThreadDigests ??= new Map()).set(doc.threadId, digest) }   // a re-issue's fresh thread is used now (4.1) — and remembered with its document
  ...
  if (res?.counterPending) {
    ...
    try { res.counterOut = await counterEffect(p, card.anchor, when, {}, [own]) } catch (err) { say(p, `Gegenschritt-Dokument: ${String((err as any)?.message ?? err)} — bleibt ausstehend`) }
  } else if (res) delete res.counterPending
  return res
}
...
function deliveryChecks (p: Person, ctx: any, doc: any): any {
  const pl = doc.payload
  if (!schemaOk('payload-encounter-credential-delivery.schema.json', pl)) return { error: 'malformed credential delivery' }
  const cred = pl.credential
  if (!C.calOK(doc.issuedAt)) return { error: 'calendar-invalid issuedAt' }
  if (doc.issuer !== cred.issuer) return { error: 'outer binding (issuer = credential.issuer)' }
  if (cred.credentialSubject?.id !== doc.recipient) return { error: 'outer binding (recipient = subject)' }
  if (doc.ceremony?.enactment !== undefined && !sameDigest(doc.ceremony.enactment, cred.credentialSubject.enactmentBinding)) return { error: 'ceremony.enactment recomputes not (validation-failed)' }
  ...
  if (!tuple?.channel?.own || tuple.channel.own.anchor !== ctx.anchor) return { error: 'validation-failed (no relationship channel to the issuer)' }
  ...
  if (step === 'counter' && !isBundleThread) return { error: 'validation-failed (counter-step outside the bundle thread)' }
  if (step === 'deliver' && isBundleThread) return { error: 'validation-failed (standalone delivery inside the bundle thread)' }
  if (!isBundleThread && threadInUse(p, doc.threadId)) return { error: 'validation-failed (threadId not fresh)' }
...
async function receiveDelivery (p: Person, ctx: any, doc: any, digest: string, when: number, ent: any) {
  const pre = deliveryChecks(p, ctx, doc)
  if (pre.error) return failed(pre.error)
  const buffered: any = await effect<any, any>(p, [digest], {
    prepare: async () => {
      if (cached(p, digest)) return { done: dupResult(p, digest, pre.cred.issuer) }
...
async function acceptBuffered (p: Person, tuple: any, cred: any, credDigest: string, ctx: any, when: number): Promise<string | null> {
  ...
  if (!e) e = acceptTail(cred, rec, 'credential')
  if (!e && rec.accepted.in && !sameDigest(rec.accepted.in, credDigest)) e = 'ERR_CONFLICT'   // step 8
  tuple.buffered.delete(credDigest)   // evaluated — the buffer held it until now
  if (e) { say(p, `${tuple.name} — gepuffertes Credential nicht angenommen (${e}); der Aussteller erfährt davon nichts`); return e }
...
function carryToHead (p: Person, tuple: any) {
  if (!tuple.deactivated || tuple.state !== '✓') return
  const head = p.contacts.get(tuple.chainedInto)
  if (head && !head.deactivated) { head.state = '✓'; head.provenance = 'ceremony' }
}

// ── the host's producing call: byte-identical redelivery of unacked docs (4.2)
export function flushEncounter (p: Person) {
  const outbound: any[] = []
  for (const t of p.contacts.values()) for (const o of (t.outbox ?? new Map()).values()) {
    if ((o.kind === 'encounter-bundle' || o.kind === 'encounter-credential-delivery') && !o.acked) outbound.push({ to: t, kind: o.kind + '/0.1 (Retry, byte-identisch)', env: structuredClone(o.env) })   // the store keeps its own copy
  }
  return { outbound }
}
