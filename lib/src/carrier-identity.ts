...
const CC_15 = (cp: number): boolean => cp <= 0x1f || (cp >= 0x7f && cp <= 0x9f)
const CF_15_RANGES: Array<[number, number]> = [
...
const CF_15 = (cp: number): boolean => CF_15_RANGES.some(([a, b]) => cp >= a && cp <= b)
export function validCarrierIdentifier (c: unknown): c is string {
  ...
  for (let i = 0; i < c.length; i++) {
    ...
    if (u >= 0xdc00 && u <= 0xdfff) return false                       // lone low
    if (u >= 0xd800 && u <= 0xdbff) {
      const nxt = c.charCodeAt(i + 1)
      if (!(nxt >= 0xdc00 && nxt <= 0xdfff)) return false              // lone high
      i++
    }
  }
  const bytes = new TextEncoder().encode(c)
  if (bytes.length < 1 || bytes.length > 1024) return false
  for (const ch of c) {
    const cp = ch.codePointAt(0)!
    if (CC_15(cp) || CF_15(cp) || WHITE_SPACE_15.has(cp)) return false
  }
  return true
}

export interface CarrierPrincipal {
  principal: string          // did:key:z6Mk…, the control principal
  key: KeyPair               // its Ed25519 pair (for the proofs of §5a.3)
  info: string               // the derivation info, for auditability
  keyAgreement: null         // declared, not omitted: Ed25519-only (§7a.1)
}
...
export async function carrierPrincipal (
