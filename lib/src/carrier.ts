...
// ── declared constants (guarantee 1: the declaration binds) ─────────────
export interface CarrierDeclaration {
  carrier: string                 // C, byte-exact (Identity §7a.2)
  orphanHorizonMs: number         // [P7D, P365D]
  giveUpHorizonMs: number         // [P1D, P90D]
  challengeLifetimeMs: number     // [PT5S, PT5M]
  queueFloorBytes: number         // [65536, maxQueueBytes]
  maxQueueBytes: number           // [1048576, 1073741824]
  maxBindingTombstones: number    // [1, 2^53−1]
  statusHorizonMs: number         // [PT5S, PT5M]
}

const DAY = 86_400_000
const DOMAINS: Array<[keyof CarrierDeclaration, number, number]> = [
...
export function validateDeclaration (d: CarrierDeclaration): string[] {
  const errs: string[] = []
  if (!validCarrierIdentifier(d.carrier)) errs.push('carrier identifier violates Identity 7a.2')
  for (const [k, lo, hi] of DOMAINS) {
    const v = d[k] as number
    if (!Number.isSafeInteger(v) || v < lo || v > hi) errs.push(`${k} outside [${lo}, ${hi}]`)
  }
  ...
  if (d.orphanHorizonMs < d.giveUpHorizonMs) errs.push('orphan-horizon < give-up-horizon')
  return errs
}
...
export interface CarrierPolicy {
  /** shared admission gate — the one unknown-address traffic competes
   * for. NEVER invoked for a request naming a held binding. */
  admitNew?: () => boolean
  /** per-binding gate for requests naming a HELD rkid (guarantee 4). */
  admitHeld?: (rkid: string) => boolean
  /** per-queue submission meter — the only resource hook reachable below
   * the floor (guarantee 5); DO-6 lives here. A duplicate charges it
   * like an admitted submission (§5a.5) and stores nothing. */
  meterQueue?: (rkid: string, bytes: number) => boolean
  /** global occupancy — s6, unreachable below the floor. */
  hasCapacity?: (bytes: number) => boolean
  /** room for one more binding — r4, never consulted for a held one. */
  hasBindingRoom?: () => boolean
}

// ── verdicts ────────────────────────────────────────────────────────────
export type RegistrationVerdict =
  | { verdict: 'registered' | 'registered(idempotent)' | 'rebound' }
  | { verdict: 'refused(malformed)' | 'refused(possession-failed)' }
  | { verdict: 'refused(stale-generation)', heldGeneration: number }   // §5a.3: the disclosure
  | { verdict: 'registration-refused(capacity)' | 'refused(admission-resource)', retriable: true }

export type SessionVerdict =
  | { verdict: 'served' }
  | { verdict: 'refused(no-such-queue)' | 'refused(malformed)' | 'refused(possession-failed)' }
  | { verdict: 'refused(admission-resource)', retriable: true }

export type SubmissionVerdict =
  | { verdict: 'admitted' | 'duplicate' }
  | { verdict: 'refused(no-such-queue)' | 'refused(bounds)' }
  | { verdict: 'refused(admission-resource)' | 'refused(queue-saturated)' | 'refused(capacity)', retriable: true }

// ── the proof object (§5a.3, wire 0.3) ──────────────────────────────────
export interface CarrierProof {
  v: string, type: string
  purpose: 'register' | 'rebind' | 'collect' | 'conclude'
  carrier: string, principal: string, rkid: string
  generation?: number
...
export const canonicalGenerationSpelling = (raw: string): boolean =>
  /^[1-9][0-9]*$/.test(raw) && Number.isSafeInteger(Number(raw))

const canonicalB64u32 = (s: unknown): boolean => {
  if (typeof s !== 'string' || s.length !== 43) return false
  try { const b = fromB64u(s); return b.length === 32 && b64uOf(b) === s } catch { return false }
}

// decode-and-check: schema validity is not acceptance (§11)
const decodedEd = (did: unknown): Uint8Array | null => {
...
const decodedX = (mk: unknown): Uint8Array | null => {
...
const cmpRkidBytes = (a: string, b: string): number => {
  const ka = decodedX(a); const kb = decodedX(b)
  if (!ka || !kb) return a < b ? -1 : 1
  for (let i = 0; i < 32; i++) if (ka[i] !== kb[i]) return ka[i]! - kb[i]!
  return 0
}
const decodedSig = (zsig: unknown): Uint8Array | null => {
  if (typeof zsig !== 'string' || zsig[0] !== 'z') return null
...
export function checkProofShape (
  p: CarrierProof, expectedCarrier: string, rawGeneration?: string,
): 'refused(malformed)' | null {
  if (typeof p !== 'object' || p === null) return 'refused(malformed)'
  if (p.v !== PROOF_V || p.type !== 'carrier-registration-proof') return 'refused(malformed)'
  if (!['register', 'rebind', 'collect', 'conclude'].includes(p.purpose)) return 'refused(malformed)'
  if (p.carrier !== expectedCarrier) return 'refused(malformed)'
  ...
  for (const k of Object.keys(p)) if (!allowed.has(k)) return 'refused(malformed)'
  if (!decodedEd(p.principal) || !decodedX(p.rkid) || !decodedSig(p.sig)) return 'refused(malformed)'
  if (!canonicalB64u32(p.principalChallenge)) return 'refused(malformed)'
  if (sessionScoped) {
    if (p.generation !== undefined || p.addressChallenge !== undefined) return 'refused(malformed)'
  } else {
    if (!Number.isSafeInteger(p.generation) || (p.generation as number) < 1) return 'refused(malformed)'
    if (rawGeneration === undefined || !canonicalGenerationSpelling(rawGeneration) ||
        Number(rawGeneration) !== p.generation) return 'refused(malformed)'
    if (!canonicalB64u32(p.addressChallenge)) return 'refused(malformed)'
  }
  return null
}
...
async function verifySignature (p: CarrierProof): Promise<boolean> {
  const pub = decodedEd(p.principal)!; const sig = decodedSig(p.sig)!
  const { sig: _omit, ...signed } = p
  const key = await S.importKey('raw', pub as Uint8Array<ArrayBuffer>, { name: 'Ed25519' }, false, ['verify'])
  return S.verify({ name: 'Ed25519' }, key, sig as Uint8Array<ArrayBuffer>, te.encode(jcs(signed as unknown as Json)))
}

/** compatibility shim for the shape+signature question alone (tests,
 * tooling): stateless, no challenge semantics. */
export async function verifyProofShape (
  p: CarrierProof, expectedCarrier: string, rawGeneration?: string,
): Promise<'refused(malformed)' | 'refused(possession-failed)' | null> {
  const sessionScoped = p?.purpose === 'collect' || p?.purpose === 'conclude'
...
const canonicalB64uAny = (s: unknown): Uint8Array | null => {
  if (typeof s !== 'string' || s.length === 0) return null
  try { const b = fromB64u(s); return b64uOf(b) === s ? b : null } catch { return null }
}
export function checkEnvelopeBytes (bytes: Uint8Array, rkid: string): boolean {
  let doc: unknown
  try { doc = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(bytes)) } catch { return false }
  if (typeof doc !== 'object' || doc === null) return false
  const e = doc as Record<string, unknown>
  if (Object.keys(e).length !== 4) return false
  if (e.rkid !== rkid) return false
  const epk = canonicalB64uAny(e.epk); const nonce = canonicalB64uAny(e.nonce)
...
// ── state ───────────────────────────────────────────────────────────────
interface Deposit { bytes: Uint8Array, digest: string, admittedAt: number, lifeEndsAt: number }
interface Binding {
  state: 'live' | 'closing'
  generation: number
  principal: string
  ...
  deadline?: number            // = closing instant + give-up-horizon; it does not move
}
interface Tombstone { generation: number, releaseSeq: number }

export interface CarrierEvents {
  onRelease?: (rkid: string, tombstoneGeneration: number) => void
  onGiveUp?: (rkid: string, digest: string) => void
  onEvict?: (rkid: string) => void      // a local hook; the port reports eviction to no one
}

interface Challenge { rkid: string, addressValue: string | null, expiresAt: number }

export class Carrier {
  #decl: CarrierDeclaration
  ...
  get decl (): CarrierDeclaration { return { ...this.#decl } }
  private policy: CarrierPolicy
  private events: CarrierEvents
  ...
  constructor (decl: CarrierDeclaration, policy: CarrierPolicy = {}, events: CarrierEvents = {}) {
    const errs = validateDeclaration(decl)
    if (errs.length) throw new Error(`declaration rejects (§11): ${errs.join('; ')}`)
    this.#decl = { ...decl }; this.policy = policy; this.events = events
  }
  ...
  private advance (rawNow: number): void {
    ...
    if (!Number.isFinite(rawNow) || rawNow < 0) throw new Error('now must be a finite non-negative instant')
    ...
    for (const [rkid, b] of this.bindings) {
      if (b.state === 'live') {
        ...
        for (const d of b.deposits.filter((d) => now >= d.lifeEndsAt)) this.events.onGiveUp?.(rkid, d.digest)
        ...
        if (now >= closingAt) {
          ...
          for (const d of b.deposits) d.lifeEndsAt = Math.min(d.lifeEndsAt, b.deadline)
        }
      }
    }
    ...
    const due: Array<{ rkid: string, b: Binding }> = []
    for (const [rkid, b] of this.bindings) {
      if (b.state === 'closing' && now >= (b.deadline as number)) due.push({ rkid, b })
    }
    ...
    for (const { rkid, b } of due) {
      ...
      for (const d of b.deposits) this.events.onGiveUp?.(rkid, d.digest)
      this.bindings.delete(rkid)
      this.installTombstone(rkid, b.generation, b.deadline as number)
      this.events.onRelease?.(rkid, b.generation)
    }
    for (const [id, ch] of this.challenges) if (now >= ch.expiresAt) this.challenges.delete(id)
    this.pendingSeqInstant = null
  }
  tick (now: number): void { this.advance(now) }
  ...
  issueChallenge (
    ...
    if (entropy.length !== 32) throw new Error('challenge entropy is 32 bytes')
  ...
  private consume (value: string, rkid: string, now: number): Challenge | null {
    const ch = this.challenges.get(value)
    this.challenges.delete(value)
    if (!ch || Math.max(now, this.clock) >= ch.expiresAt) return null
    if (ch.rkid !== rkid) return null                    // bound, always (B-1)
    return ch
  }

  // ── registration / rebind: r1–r5 (§5a.3, rounds 43/44) ───────────────
  async register (proof: CarrierProof, opts: { now: number, rawGeneration: string }): Promise<RegistrationVerdict> {
    ...
    if (p?.purpose !== 'register' && p?.purpose !== 'rebind') return { verdict: 'refused(malformed)' }
    if (checkProofShape(p, this.#decl.carrier, opts.rawGeneration)) return { verdict: 'refused(malformed)' }
    ...
    if (!admitted) return { verdict: 'refused(admission-resource)', retriable: true }
    ...
    if (!ch) return { verdict: 'refused(possession-failed)' }
    ...
    if (!await verifySignature(p)) return { verdict: 'refused(possession-failed)' }
    ...
    if ((held !== undefined) !== heldAtGate) {
      ...
      if (!readmit) return { verdict: 'refused(admission-resource)', retriable: true }
    }
    ...
    // r5 — the state table
    const g = p.generation as number
    const tomb = this.tombstones.get(p.rkid)
    if (held) {
      if (g > held.generation) {
        held.generation = g; held.principal = p.principal
  ...
  private freshBinding (generation: number, principal: string, now: number): Binding {
    return { state: 'live', generation, principal, deposits: [], lastCollectionAt: now }
  }
  private reopen (b: Binding, now: number): void {
    if (b.state === 'closing') {
      b.state = 'live'; b.deadline = undefined
      // the return ends the wind-up: deposits revert to their own
      // admission-dated horizons (§5a.9)
      for (const d of b.deposits) d.lifeEndsAt = d.admittedAt + this.#decl.giveUpHorizonMs
    }
  ...
  async collect (proof: CarrierProof, opts: { now: number }): Promise<SessionVerdict & { deposits?: Array<{ bytes: Uint8Array, digest: string }> }> {
    const p = { ...proof }
    const s = await this.session(p, 'collect', opts.now)
    if (s) return s
    const b = this.bindings.get(p.rkid)
    if (!b || b.principal !== p.principal) return { verdict: 'refused(no-such-queue)' }
    b.lastCollectionAt = this.clock
    return { verdict: 'served', deposits: b.deposits.map((d) => ({ bytes: d.bytes.slice(), digest: d.digest })) }
  }

  async conclude (proof: CarrierProof, digest: string, opts: { now: number }): Promise<SessionVerdict> {
    const p = { ...proof }
  ...
  private async session (proofArg: CarrierProof, expected: 'collect' | 'conclude', now: number): Promise<SessionVerdict | null> {
    const p = { ...proofArg }
    this.advance(now)
    if (p.purpose !== expected) return { verdict: 'refused(malformed)' }
    if (checkProofShape(p, this.#decl.carrier)) return { verdict: 'refused(malformed)' }
    const held = this.bindings.get(p.rkid)
    ...
    this.advance(now)
    if (!this.bindings.has(p.rkid)) return { verdict: 'refused(no-such-queue)' }
    return null
  }

  // ── submission: s1–s6 (§5a.5) ────────────────────────────────────────
  async submit (rkid: string, callerBytes: Uint8Array, opts: { now: number }): Promise<SubmissionVerdict> {
    ...
    if (!this.bindings.has(rkid)) return { verdict: 'refused(no-such-queue)' }
    ...
    if (!b) return { verdict: 'refused(no-such-queue)' }
    ...
    if (!withinFloor) {
      ...
      if (occupancy > decl.maxQueueBytes) return { verdict: 'refused(queue-saturated)', retriable: true }
  ...
  private pendingSeqInstant: { instant: number, seq: number } | null = null
  private installTombstone (rkid: string, generation: number, releasedAt: number): void {
  ...
  private trimTombstones (bound: number): void {
    while (this.tombstones.size > bound) {
      const victim = [...this.tombstones.entries()].sort((a, b) =>
        (a[1].releaseSeq - b[1].releaseSeq) || cmpRkidBytes(a[0], b[0]))[0][0]
  ...
  reviseDeclaration (next: Partial<CarrierDeclaration>): void {
    ...
    if (errs.length) throw new Error(`revision rejects (§11): ${errs.join('; ')}`)
    this.#decl = revised
    this.trimTombstones(revised.maxBindingTombstones)
  }

  // ── introspection (tests and adapters; not port surface) ─────────────
  stateOf (rkid: string): 'unbound' | 'live' | 'closing' | 'released' {
    const b = this.bindings.get(rkid)
    if (b) return b.state
    return this.tombstones.has(rkid) ? 'released' : 'unbound'
  }
  tombstoneOf (rkid: string): number | null { return this.tombstones.get(rkid)?.generation ?? null }
  bindingOf (rkid: string): { generation: number, principal: string, deposits: number, deadline?: number } | null {
    const b = this.bindings.get(rkid)
    return b ? { generation: b.generation, principal: b.principal, deposits: b.deposits.length, deadline: b.deadline } : null
  }
  tombstoneLabels (): string[] { return [...this.tombstones.keys()] }
}
