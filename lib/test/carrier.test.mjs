...
test('R5 B-1: a malformed rkid at the challenge entry buys neither gate nor entropy', async () => {
  let gated = 0, drawn = 0
  const c = new Carrier(DECL, { admitNew: () => { gated++; return true } })
  assert.throws(() => c.issueChallenge(0, 'not-an-rkid', () => { drawn++; return { entropy: chalBytes(1) } }),
    /malformed/)
  assert.equal(gated, 0, 'syntax precedes admission')
  assert.equal(drawn, 0, 'syntax precedes randomness — §5a.3 order, measured')
})
